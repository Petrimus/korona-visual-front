...
const App = () => {
    ...
    const [districtToShow, setDistrictToShow] = useState(
    ...
    useEffect(() => {
        if (
            infectionData === null ||
            hospitalisedData === null ||
            exitusData === null
        ) {
            return
        }
        setTopViewData(
            functionUtils.contructTopViewData(
                infectionData,
                hospitalisedData,
                exitusData
            )
    ...
    const dataSelect = () => {
        ...
        if (viewSelect === 'infections') {
            ...
            return hospitalisedData.slice(
                hospitalisedData.length - 20,
                hospitalisedData.length
            )
        } else if (viewSelect === 'exitus') {
            if (!cumulativeExitus) {
                return functionUtils.arrangeDataToSingleValue(exitusData)
            } else {
                return functionUtils.arrangeCumulativeValueChart(
                    exitusData.sort()
                )
            }
        }
    ...
    const handleCumulativeButtonClick = (name) => () => {
        if (name === 'infections') {
            setCumulativeInfections(!cumulativeInfections)
        }
        if (name === 'exitus') {
            setCumulativeExitus(!cumulativeExitus)
        }
    }
