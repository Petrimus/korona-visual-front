...
const getDiffMovingAvg = (data) => {
    let lastIndex = lastIndexNotZero(data)
    ...
    if (lastIndex > 14) {
        for (let i = 0; i < 7; i++) {
            sum1 = sum1 + data[lastIndex - i].value
        }

        for (let j = 7; j < 14; j++) {
            sum2 = sum2 + data[lastIndex - j].value
        }
...
const contructTopViewData = (infection, hospi, exit) => {
    ...
    const allDictrictsInfection =
        infection.confirmed['Kaikki sairaanhoitopiirit']
    const ary = []
